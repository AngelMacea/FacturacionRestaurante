...
END
GO
CREATE PROCEDURE Gnrl.UDP_tblCiudades_Update
    @Ciud_Id    INT,
    @Ciud_Descripcion NVARCHAR(50),
    @Pais_Id    INT,
...
AS
BEGIN
	SELECT	Prov.Prov_Id AS [ProveedorId],
			Prov.Prov_Descripcion AS [Proveedor],
			Prov.Prov_Tel AS [ProveedorTel],
...
BEGIN
	...
	INNER JOIN Gnrl.tblCiudades AS Ciud ON Ciud.Ciud_Id = Prov.Prov_Ciudad
	INNER JOIN Gnrl.tblPaises AS Pais  ON Pais.Pais_Id = Ciud.Pais_Id
	WHERE Prov.Prov_Estado = 1 AND Prov_Id = @Prov_Id
END
GO
--Tabla Clientes 
...
AS
BEGIN
	SELECT Menu_Id AS [ID], Menu_Descripcion [Descripcion], Menu_Precio AS [Precio] FROM Gnrl.tblMenus
	WHERE Menu_Estado = 1
END
GO
...
AS
BEGIN
	 SELECT Comp_Id ID FROM Inv.tblCompras WHERE Comp_NoOrden = @NumOrden
END
GO
...
AS
BEGIN
	SELECT CODE.CompDe_Id ,COMP.Comp_NoOrden ,INGR.Ingr_Id , INGR.Ingr_Descripcion,
	CODE.CompDe_PrecioCompra , CODE.CompDe_Cantidad,COMP.Comp_IVA
	FROM Inv.tblCompraDetalles AS CODE
	INNER JOIN Inv.tblCompras AS COMP ON COMP.Comp_Id = CODE.Comp_Id
	INNER JOIN Inv.tblIngredientes AS INGR ON INGR.Ingr_Id = CODE.Ingr_Id
	WHERE CODE.CompDe_Estado = 1 AND COMP.Comp_NoOrden = @Comp_NoOrden
